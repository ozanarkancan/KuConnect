...
def initialize_weights(n_in, n_hidden, bias=False, init="normal", scale=0.01, n_out = None):
    ...
    if bias:
        ...
        params += [b_hh]
        if n_out != None:
            b_hy = zeros(n_out, name='b_hy')
            params += [b_hy]

    return params
    
def initialize_weights_feedback(n_in, n_hidden, n_out, feedback=1, bias=False, init="normal", scale=0.01):
    """
    Initialize weights, use normal or uniform distribution
    """
    if init == "normal":
    ...
    if feedback == 2:
        W_yh2 = init_func(n_out, n_hidden, name='W_yh2', scale = scale)
        params += [W_yh2]
...
class Elman(object):
    def __init__(self, input, d_input, n_in, n_hidden, h0=None, d_h0=None, activation="tanh", bias=True,
        ...
        if bias:
            self.W_ih, self.W_hh, self.b_hh = initialize_weights(n_in, n_hidden,
                bias, init, scale)
...
class ElmanFeedback2(object):
    def __init__(self, input, d_input, n_in, n_hidden, n_out, h0=None, d_h0=None, activation="tanh", bias=True,
        ...
        if bias:
            self.W_ih, self.W_hh, self.W_hy, self.W_yh, self.W_yh2, self.b_hh, self.b_hy = initialize_weights_feedback(n_in,
                n_hidden, n_out, 2, bias, init, scale)
            self.params = [self.W_ih, self.W_hh, self.W_hy, self.W_yh, self.W_yh2,
                self.b_hh, self.b_hy]
        else:
            self.W_ih, self.W_hh, self.W_hy, self.W_yh, self.W_yh2 = initialize_weights_feedback(n_in,
                n_hidden, n_out, 2, bias, init, scale)
        ...
        self.y0 = theano.shared(np.zeros((n_out, 3), dtype='float32'), 'y0', borrow=True)
        self.d_y0 = theano.shared(np.zeros((n_out, 3), dtype='float32'), 'd_y0', borrow=True)
        
        def step(x_t, h_tm1, y_tm2, y_tm1):
            tot = T.dot(x_t, self.W_ih) + T.dot(h_tm1, self.W_hh) + T.dot(y_tm1, self.W_yh) + T.dot(y_tm2, self.W_yh2)
            h_t = self.act(tot + self.b_hh) if bias else self.act(tot)
        ...
        def d_step(x_t, h_tm1, y_tm2, y_tm1):
            tot = T.dot(x_t, self.W_ih) + T.dot(h_tm1, self.W_hh) + T.dot(y_tm1, self.W_yh) + T.dot(y_tm2, self.W_yh2)
            h_t = self.act(tot + self.b_hh) if bias else self.act(tot)
            d_h_t = dropout(h_t, dropout_rate)
            tot = T.dot(d_h_t, self.W_hy)
        ...
        [self.d_h, self.d_y], _ = theano.scan(d_step,
            sequences=self.d_input,
            outputs_info=[self.d_h0, 
                dict(initial=self.d_y0, taps=[-2, -1])],
            n_steps=self.d_input.shape[0],
            truncate_gradient=truncate)
        ...
        self.output = self.y
        self.d_output = self.d_y
        self.memo = [(self.h0, self.h[-1]), (self.y0, self.y[-3:])]
