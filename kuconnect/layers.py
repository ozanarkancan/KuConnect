...
class Recurrent2OutputLayer(object):
    def __init__(self, input, d_input, n_in, n_out, bias=True, losstype="softmax"):
        ...
        self.y0 = theano.shared(np.zeros((2, n_out), dtype='float32'), name='y0')
        self.d_y0 = theano.shared(np.zeros((2, n_out), dtype='float32'),
            name='d_y0')
        
        def step(x_t, y_tm2, y_tm1):
        ...
        self.output, _ = theano.scan(step,
            sequences=input,
            outputs_info=[dict(initial=self.y0, taps=[-2, -1])],
            n_steps=input.shape[0])
        
        self.d_output, _ = theano.scan(step,
            sequences=d_input,
            outputs_info=[dict(self.d_y0, taps=[-2, -1])],
            n_steps=d_input.shape[0])
        ...
        if losstype == "softmax":
            ...
            raise ValueError('Unknown loss')
        self.params = [self.W, self.W_r, self.W_r2, self.b]



class BidirectionalRecurrent2OutputLayer(object):
    def __init__(self, f_input, f_d_input, b_input, b_d_input, n_in, n_out, bias=True, losstype="softmax"):
        self.n_in = n_in
        self.n_out = n_out
        ...
        def step(f_x_t, b_x_t, y_tm2, y_tm1):
            y_t = T.dot(f_x_t, self.W_f) + T.dot(b_x_t, self.W_b) + \
                T.dot(y_tm1, self.W_r) + T.dot(y_tm2, self.W_r2) + self.b
            if losstype == "softmax":
        ...
        self.d_output, _ = theano.scan(step,
            sequences=[f_d_input, b_d_input],
            outputs_info=[dict(initial=self.d_y0, taps=[-2,-1])],
            n_steps=f_input.shape[0])
        ...
        if losstype == "softmax":
            ...
            raise ValueError('Unknown loss')
        self.params = [self.W_f, self.W_b, self.W_r, self.W_r2, self.b]
